...
function SentimentGraph(props) {
    ...
    // const allJournalEntries = [{"id":18,"content":"test1","zipcode":null,"sentiment":0.413056568845912,"magnitude":-0.8146495621597709,
    ...
    // Calculate the start date from 2 weeks ago
    const startDate = Date.parse(new Date(Date.now() - 12096e5))

    // Filter the array for dates in the last two weeks
    const journalEntriesLastTwoWeeks = journalEntryData.filter(journalEntry => journalEntry.time > startDate)

    journalEntryData = journalEntriesLastTwoWeeks

    journalEntryData.sort((journalEntry1, journalEntry2) => journalEntry2.time - journalEntry1.time)

    return (
        <Wrapper>
            <h2>Global sentiment over time:</h2>
            <LineChart 
                width={500} 
                height={350} 
                margin={{bottom: 35, right: 10}}
                data={journalEntryData}>
                <Line type="monotone" dataKey="sentiment" stroke="red" />
                <XAxis
                    dataKey="time"
                    name = 'Time'
                    type = 'number'
